...
* ``coarse/`` holds ``cue6``, ``onoff_reliable`` and ``coarse3_reliable`` at one
  ...
  was a 2-class model at 0.786. Ranking by that made it the dashboard DEFAULT
...
def test_a_coarse_model_never_becomes_the_default(tmp_path):
    """It scores highest and must still lose: 0.786 over 2 classes at 91%
...
def test_the_canonical_variant_id_is_unchanged(tmp_path):
    ...
    calibration filenames and every id quoted in FINDINGS resolve by this
