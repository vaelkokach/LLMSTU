...
(FINDINGS 21). It cannot be asked for a phone. The pretrained one can: the same
test returns **0** detections for the nonsense prompt and ``cell phone`` boxes an
order of magnitude smaller than person boxes.

Which features
--------------
The three that survived matched-pair validation (FINDINGS 21.4) --
``score_contained`` (AUROC 0.827), ``y_frac`` (0.892) and their product
(0.958) -- per object, and only those. See ``attention/object_features.py``.
