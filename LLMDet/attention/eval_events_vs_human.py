...
Background (FINDINGS.md 6): the previously reported 89% episode-miss rate
...
def load_tracks(manifest, annotations, crops_root):
    """(video_id, seat_id) -> [(t, crop_path, teacher_cue, human_cue_or_None)]."""
    ann = {}
    ...
    for line in open(manifest):
        ...
        tracks[(m["video_id"], str(m["seat_id"]))].append(
            (float(m["t"]), str(Path(crops_root) / m["file_name"]),
             map_record(m), human))
    for k in tracks:
...
def main():
    ...
    tracks = load_tracks(args.manifest, args.annotations, args.crops_root)
    ...
    model = AttentionTransformer(
        ...
        max_seq_len=int(cfg["model"]["max_seq_len"])).to(device)
    ...
    major = Counter(c for rows in tracks.values() for _, _, c, _ in rows).most_common(1)[0][0]
    ...
    for key, rows in sorted(tracks.items()):
        ...
        for _, path, _, _ in rows:
            im = cv2.imread(path)
            if im is None:
                feats.append(np.zeros(feat.output_dim(), dtype=np.float32))
                continue
            h, w = im.shape[:2]
            feats.append(feat.extract_batch(im, [[0, 0, w - 1, h - 1]])[0])
        x = torch.from_numpy(np.stack(feats)[None]).float().to(device)
        with torch.inference_mode():
            logits = model(x)
        pred = logits[0].argmax(-1).cpu().numpy()
        if pred.shape[0] != len(rows):  # model may emit a single sequence label
            pred = np.full(len(rows), int(np.atleast_1d(pred)[0]), dtype=np.int64)
        ...
        teacher = np.array([r[2] for r in rows], dtype=np.int64)
        for i, r in enumerate(rows):
            if r[3] is not None:  # human-verified frames only
                frame_model.append(pred[i]); frame_teacher.append(teacher[i])
                frame_human.append(r[3])
    ...
    all_t = [t for rows in tracks.values() for t, _, _, _ in rows]
