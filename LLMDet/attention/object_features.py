...
(FINDINGS 21). It cannot be re-prompted to find phones.
...
magnitude smaller than person boxes with 0% overlap (FINDINGS 21.1). Obj365
...
feature              AUROC    
===================  =======  ==============================================
``score * y_frac``   0.958    score weighted by how far down the box it sits
``y_frac``           0.892    a held phone is at hand/desk height, not head
``score_contained``  0.827    the detector's confidence, contained in the box
-------------------  -------  ----------------------------------------------
``rel_area``         0.556    no signal -- dropped
``n_contained``      0.270    INVERTED: counts how busy the desk is -- dropped
===================  =======  ==============================================
...
An earlier version of this measurement gave ``y_frac`` an AUROC of 0.994, which
...
def student_object_features(
    ...
    of students who had no phone (FINDINGS 21.2).
    """
    ...
    for oi, (boxes, scores) in enumerate(boxes_by_object):
        ...
        for b, s in zip(np.asarray(boxes, dtype=np.float64).reshape(-1, 4),
            ...
            if s > best_s:
                ...
                # a real phone sits at 0.656, the detector's false positives at
                # 0.241 -- head height.
        ...
        # dropped: without it this column would be the 0.994 artefact again.
        out[o + 1] = best_y
