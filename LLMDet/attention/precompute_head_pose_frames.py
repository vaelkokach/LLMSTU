...
``bbox_crop`` is a **tighter sub-region** of ``bbox_person`` — median IoU 0.706,
contained in the person box but never containing it (FINDINGS 11.14).
...
~80% of the head-pose contribution (FINDINGS 11.10). Letterboxing the runtime
...
to ``bbox_person`` is nearly fixed (bottom offset -0.001 +/- 0.035) but the
horizontal trim is not (width ratio 0.825 +/- 0.221), so no rule recoverable
